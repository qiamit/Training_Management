...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      return json(
        { error: "Only Quality International staff can create individuals." },
        403,
      );
    }

    const body = (await req.json()) as CreateIndividualBody;
    ...
    const { data: created, error: createError } =
      await admin.auth.admin.createUser({
        ...
        user_metadata: {
          ...
          date_of_birth: body.dateOfBirth?.trim() ?? "",
          city: body.city?.trim() ?? "",
    ...
    if (createError || !created.user) {
      return json(
        { error: createError?.message ?? "Failed to create individual user." },
        400,
      );
    }
